...
def SerialSearch_AxC2(rfdata:np.array, interFrequency:float, code:np.array, dopplerRange:tuple, dopplerStep:int, 
                     samplingFrequency:float, samplesPerCode:int, axc_mult, n_bits:int):
    """
    """
    ...
    for idxCode in range(len(code)):    
        ...
        # Multiply signal with code
        signal = np.zeros_like(rfdata)
        for i in range(len(rfdata)):
            signal[i] = axc_mult(_code[i], rfdata[i])
        ...
        for idxFreq in range(len(frequencyBins)):
            ...
            q_signal = np.zeros_like(q_carrier)
            for i in range(len(i_carrier)):
                i_signal[i] = axc_mult(i_carrier[i], signal[i])
                q_signal[i] = axc_mult(q_carrier[i], signal[i])
